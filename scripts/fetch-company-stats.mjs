...
function pack(raw) {
  // raw: { three_months: [...], six_months: [...], more_than_six_months: [...] }
  const pick = (arr) =>
    (arr || []).map((x) => [x.name, x.slug, x.timesEncountered]);
  return {
    a: pick(raw?.three_months),
    b: pick(raw?.six_months),
    m: pick(raw?.more_than_six_months),
  };
}
...
async function fetchStats(slug) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      const raw = j.data?.question?.companyTagStatsV2;
      if (!raw) return { a: [], b: [], m: [] };
      return pack(typeof raw === 'string' ? JSON.parse(raw) : raw);
    } catch (e) {
...
async function main() {
  ...
  async function worker() {
    while (cursor < todo.length) {
      ...
      try {
        const cs = await fetchStats(slug);
  ...
  console.log('');
  console.log('----------------------------------------');
  console.log(`enriched: ${ok}, failed: ${failed.length}`);
  if (failed.length) {
